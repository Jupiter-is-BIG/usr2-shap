...
# -*- coding: utf-8 -*-
"""
Temporal Alignment SHAP (usr2)

Ported from Dr-SHAP-AV's Compute_Alignment_SHAP.py. usr2's num_audio_tokens
already stores the exact N_a used in the Shapley matrix (see
shap_evaluator.py), so unlike the original script there is no need to infer
the model from the filename or divide by a downsample factor.
"""
import argparse
...
parser = argparse.ArgumentParser(
    description='Temporal Alignment SHAP Analysis (usr2)',
    formatter_class=argparse.ArgumentDefaultsHelpFormatter
)

parser.add_argument('--path-to-data', required=True, metavar='PATH',
                    help='Path to usr2 .npz file.')
parser.add_argument('--num-samples', required=True, type=int, default=20,
                    help='The number of samples to use. By default, we use the 20 longest ones.')
...
# =============================================================================
print("=" * 60)
print("TEMPORAL ALIGNMENT SHAP ANALYSIS (usr2)")
print("=" * 60)
...
axes1[0].set_title('Audio', fontsize=20, fontweight='bold')
...
axes1[1].set_title('Video', fontsize=20, fontweight='bold')
...
plt.tight_layout()
plt.savefig('usr2_temporal_alignment_heatmap.pdf', dpi=400, bbox_inches='tight')
print("\nSaved: usr2_temporal_alignment_heatmap.pdf")
...
plt.tight_layout()
plt.savefig('usr2_early_vs_late_features.pdf', dpi=400, bbox_inches='tight')
print("Saved: usr2_early_vs_late_features.pdf")
